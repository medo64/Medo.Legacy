...
namespace Medo.Data {
	...
	/// </summary>
	public class DeleteCommand : System.Data.IDbCommand {
		...
		private void UpdateCommandText() {
			if (string.IsNullOrEmpty(this._whereText)) {
				this._baseCommand.CommandText = string.Format(System.Globalization.CultureInfo.InvariantCulture, "DELETE FROM {0};", this._tableName);
			} else {
				this._baseCommand.CommandText = string.Format(System.Globalization.CultureInfo.InvariantCulture, "DELETE FROM {0} WHERE {1};", this._tableName, _whereText.ToString());
			}
		}
		...
		private void DebugCommand() {
            System.Diagnostics.Debug.WriteLine(string.Format(CultureInfo.InvariantCulture, "I: {0}.    {{Medo.Data.DeleteCommand}}", this._baseCommand.CommandText));
			for (int i = 0; i < this._baseCommand.Parameters.Count; ++i) {
				...
				if (curr != null) {
                    System.Diagnostics.Debug.WriteLine(string.Format(CultureInfo.InvariantCulture, "I:     {0}=\"{1}\".    {{Medo.Data.DeleteCommand}}", curr.ParameterName, curr.Value));
				} else {
                    System.Diagnostics.Debug.WriteLine(string.Format(CultureInfo.InvariantCulture, "I:     {0}.    {{Medo.Data.DeleteCommand}}", this._baseCommand.Parameters[i].ToString()));
				}
			}
		}
#endif
