    ...
    public class SelectCommand : IDbCommand {
        ...
        private void UpdateCommandText() {
            ...
            if (this._topCount == 0) {
                ...
                selectPrefix = string.Format(CultureInfo.InvariantCulture, "SELECT TOP {0}", this._topCount);
            }
            ...
            if (string.IsNullOrEmpty(_orderByText)) {
                ...
                orderBySuffix = string.Format(CultureInfo.InvariantCulture, " ORDER BY {0}", this._orderByText);
            }

            if (string.IsNullOrEmpty(this._whereText)) {
                this._baseCommand.CommandText = string.Format(CultureInfo.InvariantCulture, "{2} {1} FROM {0}{3};", this._tableName, this._columnsText, selectPrefix, orderBySuffix);
            } else {
                this._baseCommand.CommandText = string.Format(CultureInfo.InvariantCulture, "{3} {1} FROM {0} WHERE {2}{4};", this._tableName, this._columnsText, _whereText, selectPrefix, orderBySuffix);
            }
        }
