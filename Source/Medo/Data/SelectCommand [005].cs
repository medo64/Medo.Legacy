...
namespace Medo.Data {
    ...
    public class SelectCommand : IDbCommand {
        ...
        private readonly string _columnsText;
        ...
        public SelectCommand(IDbConnection connection, string tableName, params string[] columns) {
            ...
            if ((columns != null) && (columns.Length > 0)) {
                ...
                _columnsText = sbColumns.ToString();
            } else {
                _columnsText = "*";
            }
        ...
        private void UpdateCommandText() {
            ...
            if (_topCount == 0) {
                ...
                selectPrefix = string.Format(CultureInfo.InvariantCulture, "SELECT TOP {0}", _topCount);
            }
            if ((Connection is SqlConnection) && _needsMonoFix) { selectPrefix = "SET LANGUAGE us_english; " + selectPrefix; }
            string orderBySuffix;
            if (string.IsNullOrEmpty(_orderByText)) {
                orderBySuffix = string.Empty;
            } else {
                orderBySuffix = string.Format(CultureInfo.InvariantCulture, " ORDER BY {0}", _orderByText);
            }

            if (string.IsNullOrEmpty(_whereText)) {
                _baseCommand.CommandText = string.Format(CultureInfo.InvariantCulture, "{2} {1} FROM {0}{3};", _tableName, _columnsText, selectPrefix, orderBySuffix);
            } else {
                _baseCommand.CommandText = string.Format(CultureInfo.InvariantCulture, "{3} {1} FROM {0} WHERE {2}{4};", _tableName, _columnsText, _whereText, selectPrefix, orderBySuffix);
            }
        }
