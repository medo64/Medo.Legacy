    ...
    public class SelectCommand : IDbCommand {
        ...
        private void UpdateCommandText() {
            ...
            if (string.IsNullOrEmpty(_whereText)) {
                BaseCommand.CommandText = string.Format(CultureInfo.InvariantCulture, "{2} {1} FROM {0}{3};", _tableName, _columnsText, selectPrefix, orderBySuffix);
            } else {
                BaseCommand.CommandText = string.Format(CultureInfo.InvariantCulture, "{3} {1} FROM {0} WHERE {2}{4};", _tableName, _columnsText, _whereText, selectPrefix, orderBySuffix);
            }
        }
