...
namespace Medo.Device {
    ...
    public class Elsidi : IDisposable {
        ...
        public void AddText(string text) {
            AppendTextToOutputBuffer(System.Text.ASCIIEncoding.ASCII.GetBytes(text));
        }
        ...
        public void AddText(char character) {
            AppendTextToOutputBuffer(System.Text.ASCIIEncoding.ASCII.GetBytes(new char[] { character }));
        }
        ...
        public void AddText(byte asciiIndex) {
            AppendTextToOutputBuffer(new byte[] { asciiIndex });
        }
        ...
        public void AddClearDisplay() {
            AppendToOutputBuffer(new byte[] { 0x0E, 0x01 });
        }
        ...
        public void AddReturnHome() {
            AppendToOutputBuffer(new byte[] { 0x0E, 0x02 });
        }
        ...
        public void AddEntryModeChange(bool incrementAddress, bool shiftWholeDisplay) {
            ...
            AppendToOutputBuffer(new byte[] { 0x0E, cmd });
        }
        ...
        public void AddCustomInstruction(byte value) {
            AppendToOutputBuffer(new byte[] { 0x0E, value});
        }
        ...
        private void AppendTextToOutputBuffer(byte[] content) {
            if ((content == null) || (content.Length == 0)) { return; }

            int oldBufferCount = _outputBuffer.Count;
            for (int i = 0; i < content.Length; ++i) {
                if ((content[i] >= 0x08) && (content[i] <= 0x0F)) {
                    this._outputBuffer.Add(0x0F); //SI
